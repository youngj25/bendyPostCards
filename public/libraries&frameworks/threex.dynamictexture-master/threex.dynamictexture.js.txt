...
THREEx.DynamicTexture.prototype.drawTextCooked = function(options){
	...
	//this method was modified from the original to support word-wrapping (instead of letter-wrapping)
	function computeMaxTextLength(text){
		var words = text.split(' ');
		var maxWidth = (1-params.margin*2)*canvas.width + context.measureText('--').width;

		//handle first word separately (in case there is just one word with no space)
		var textSize = context.measureText(words[0]).width;
		var maxText = words[0];

		//handle all other words
		for(var i = 1; i < words.length && textSize <= maxWidth; i++) {
			textSize += context.measureText(' ' + words[i]).width;
			if(textSize <= maxWidth) maxText += ' ' + words[i];
		}

		return maxText;
	}

	/*function computeMaxTextLength(text){ //this is the original method (which is only letter-wrap, not word-wrap)
		var maxText	= ''
